...
class OrderService:
    ...
    # =========================
    @staticmethod
    def list_orders_for_manufacturer(manufacturer_id: str):
        rows = []

        # NEW COLLECTION (preferred)
        db_new = mongo.db.farmer_orders
        new_docs = list(db_new.find({"manufacturer_id": manufacturer_id}).sort("created_at", -1))
        ...
        if not rows:
            ...
            old_docs = list(db_old.find({"farmerId": manufacturer_id}).sort("created_at", -1))
            for d in old_docs:
    ...
    # =========================
    @staticmethod
    def get_kpis(manufacturer_id: str):
        db_new = mongo.db.farmer_orders
        docs = list(db_new.find({"manufacturer_id": manufacturer_id}))
        ...
        old_docs = list(db_old.find({"manufacturerId": manufacturer_id}))
    ...
    # =========================
    @staticmethod
    def get_manufacturer_request(request_id: str):
    ...
    @staticmethod
    def get_order_for_manufacturer(manufacturer_id: str, order_id: str):
        """
        ...
        fo = mongo.db.farmer_orders.find_one(
            {"farmer_id": manufacturer_id, "order_id": order_id},
            {"_id": 0}
        )
        ...
        d = mongo.db.orders.find_one(
            {"farmerId": manufacturer_id, "orderId": order_id},
            {"_id": 0}
        )
        if not d:
            return None
